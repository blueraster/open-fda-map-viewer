...
/// ToDo: Verify logic on name of city and state for one city and state
    ...
    for(let d in data){
      ...
      let loction = encodeURIComponent(city+' '+state +' ' + country);
      //let requestURL =  geocodeBaseUrl+encodeURIComponent(loction) ;
        request.get(`https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/find?maxLocations=${maxLocations}&outSR=${outSR}&f=json&text=${loction}`)
          .then((results)=>{
            console.log(results);
          })
